...
/**
 * Get settings for a specific character, falling back to globals.
 * @param {string} avatar - Character avatar filename
 * @returns {object} Merged settings (profile overrides on top of globals)
 */
export function getCharacterSettings(avatar) {
    const s = ensureSettings();
    const profile = s.profiles[avatar];
    if (!profile) return { ...s };
    return { ...s, ...profile };
}

/**
 * Save a per-character profile override.
 * Only stores keys that differ from global defaults.
 */
export function setCharacterProfile(avatar, overrides) {
    const s = ensureSettings();
    if (!s.profiles) s.profiles = {};
    s.profiles[avatar] = overrides;
    saveSettingsDebounced();
}

/**
 * Remove a character profile (revert to global settings).
 */
export function removeCharacterProfile(avatar) {
    const s = ensureSettings();
    if (s.profiles) {
...
/**
 * Clean up profiles for characters that no longer exist.
 * @param {string[]} existingAvatars - List of current character avatar filenames
 */
export function cleanGhostProfiles(existingAvatars) {
    const s = ensureSettings();
    if (!s.profiles) return;

    const avatarSet = new Set(existingAvatars);
    let cleaned = 0;
    for (const avatar of Object.keys(s.profiles)) {
    ...
    if (cleaned > 0) {
        log(`Cleaned ${cleaned} ghost profile(s)`);
        saveSettingsDebounced();
    }
}
